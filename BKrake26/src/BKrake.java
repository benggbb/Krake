...
public class BKrake {
	...
	public static int Start(String input) {
		...
		// RUN
		EXIT_CODE = EXEC(commandList).getFirst();
		// Bufferflush
		Ress.Bufferflush();
		return EXIT_CODE;
	...
	private static ArrayList<ArrayList<Integer>> BineryList(List<Integer> block) {
		...
		for (int token : block) {
			...
			if (token == Token.CBRACKEDS) front_index++;
			if (token == Token.BCBRACKED) back_index++;

			// Token zum aktuellen "String" hinzufügen
			current_Methode.add(token);

			// Befehl abgeschlossen: Semikolon oder Ende eines Blocks auf oberster Ebene
			if ((token == 0 || token == Token.BCBRACKED) && front_index - back_index == 0) {
				// FormatCode aufrufen
				ret.add(current_Methode);
	...
	private static ArrayList<Integer> FormatCode(String current_Methode) {
		...
		Debug.Print_Log("---------NEW LINE---------\n" + Algorithm.printlist(Wordlist) + "\n--------------------");
			...
			for (int i = 0; i < Wordlist.size()-1; i++) {
				...
				if (Wordlist.get(i).equals( Token.DEF)) {
					found = true;
					int Type = Wordlist.get(i);
					int Name = Token.getTokenId( Wordlist.get(i + 1));
	...
	private static List<Integer> EXEC(ArrayList<ArrayList<Integer>> commandList) {
		...
		for (ArrayList<Integer> snippet : commandList) {
			...
			Debug.Print_Log(Algorithm.printlist(ret_value));
		}
	...
	public static List<Integer> Do(List<Integer> snippet) {
		...
		if(snippet.getFirst() == Token.DEF){
			Debug.Print_Log("[NOT EXEC: " + Token.getTokenId(snippet.getFirst()) + snippet.get(1) + "]");
		...
		if(pos == -1) {
			Debug.Print_Log(Algorithm.printlist(snippet));
	...
	public static List<Integer> Methode_Call(List<Integer> snippet){
		...
		if(Algorithm.is_Methode(Token.getTokenId( snippet.get(0)))){
			Debug.Print_Log("[EXECUTING  METHODE<<" + snippet.get(0) + ">>]\n");
			Debug.Print_Log(
					Algorithm.printlist(Ress.MethodeContent.get(Token.getTokenId(snippet.get(0))).subList(1, Ress.MethodeContent.get(Token.getTokenId( snippet.get(0))).size() -1)));
			return EXEC(BineryList(
					Ress.MethodeContent.get(
							Token.getTokenId(
									snippet.get(0))).subList(1, Ress.MethodeContent.get(Token.getTokenId( snippet.get(0))).size() -1))
			);
		}
		return snippet;
	}

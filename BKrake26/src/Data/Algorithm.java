...
public class Algorithm {
	...
	public static ArrayList<Integer> Create_TokenList(String command) { //obsoloete
		command += ' ';
		// Used Vars---------------------------------------------
			...
			else 
			{
				if (Ress.Special_Symbols.contains(chars[i] + "")) {
					...
					if (Cleared(current_Word, Ress.Cleared_Symbols).length() > 0) {
						// Debug.Print_Log("-"+Cleared(current_Word) + "-");
						ret.add(Token.Tokenize(Cleared(current_Word, Ress.Cleared_Symbols)));
					}
	...
	private static boolean contains(String Sequencers, char c) {
		return Sequencers.contains(c + "");
	}
	...
	public static String Cleared(String Input, String cleared) {
		...
		try {
			...
			Debug.Print_Log("[ERR]" + e);
		}
		return ret;
	}

	public static String Tokenli_toString(List<Integer> wordlist) {
		String Code = "[ ";
		for (Integer token : wordlist) {
			Code += Token.PrintToken(token) + (token != wordlist.getLast() ?" , ": "");
		}
		return Code +" ]";
	}

	public static ArrayList<Integer> get_Methode_Block(List<Integer> wordlist, int index) {
		ArrayList<Integer> block = new ArrayList<>();

		int bi = 0, fi = 0;
		for (int i = index; i < wordlist.size(); i++) {

			if (wordlist.get(i) == Token.CBRACKEDS) fi++;
			if (wordlist.get(i) == Token.BCBRACKED) bi++;

			if (fi != 0) {
				Debug.Print_Log_Flat(Token.PrintToken(wordlist.get(i)));
	...
	public static boolean is_Methode(int m){

		for(int met : Ress.Methode_Name){
			if(m == met){
				return true;
			}
		}
